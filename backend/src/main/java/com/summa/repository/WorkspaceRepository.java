...
public interface WorkspaceRepository extends JpaRepository<Workspace, String> {
    ...
    @org.springframework.data.jpa.repository.Query("SELECT COUNT(w) FROM Workspace w WHERE w.archivedAt IS NULL AND ('[' || w.domainIds || ']') LIKE ('%' || :domainId || '%') AND w.domainIds LIKE CONCAT('%\"', :domainId, '\"%')")
