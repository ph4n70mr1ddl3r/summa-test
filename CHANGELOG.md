...
**Documentation review pass 98 — consistency, correctness, completeness fixes (v2.68)**: fix fully-qualified class names across all model entities by adding proper imports for InstantToUnixEpochConverter; fix DnaReadService constructor using FQN instead of imported JdbcTemplate; fix WriteGate missing HttpStatus import; fix BackupService missing @Value import; fix SecurityConfig missing UsernamePasswordAuthenticationFilter import; remove invalid 'upgrade' ask kind from AskController (not in spec ASK-001 or schema CHECK constraint); fix RoleTemplateService filing upgrade asks with invalid kind by switching to approval; lint green, all tests pass.

**Documentation review pass 97 — consistency, correctness, completeness fixes (v2.67)**: fix JWT alg:none bypass in frontend auth check; fix buildQuery source/test mismatch by adding empty-string filter to source; fix PAT token exposure in create response; remove hardcoded weak JWT default secret from application.yml; fix AuditService duplicated JSON escaping by reusing JsonHelpers; fix AskService direction-ask handler to actually update initiative goal_ref; fix DnaDomain re-key scope to exclude org-wide admin broadcasts; add CFG-120 (workspace kind assignment) to close numbering gap in SPEC-20; add INT-012 (decision_ref live-at-write validation) to close referential integrity gap; standardize DWP-020/021 review_by→reviewed_by naming to match schema; consolidate viewer eligibility list to single-home in SPEC-01 with ORG-020 cross-reference; add missing FK indexes (pats.member_id, spawn_requests.approved_by, humans.deputy_member_id); fix FTS5 decision trigger to update status column; add backup path normalization to prevent symlink traversal; add Maven cache to cross-platform CI job; add SUMMA_CORS_ORIGINS and SUMMA_SPEND_CEILING to docker-compose; fix package.json test:backend to use test profile; fix ESLint redundant parser config; remove unused useState from Home component scope; fix AskInbox immediate cleanup cancellation bug; lint green, all tests pass.
