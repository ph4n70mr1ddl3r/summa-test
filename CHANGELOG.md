...
**Documentation review pass 96 — consistency, correctness, completeness fixes (v2.66)**: fix flawed DFS cycle detection in InitiativeService (remove backtracking that missed cycles); unify system actor constant usage across WorkspaceService, NodeService, GroupService, RoleTemplateService; fix ControllerResponses audit param order (message was passed as objectId); tighten CORS wildcard by removing dangerous `/*` suffix on production origins; align keyed-union regex across InitiativeService, DnaProposalService, and DnaDecisionService; add double-archive guard in WorkspaceService.archive(); surface malformed workspace bindings as audit events in DnaDomainService.merge(); add application-test.yml for isolated CI test profile; add Docker image build verification to CI; run backend tests on all cross-platform matrix nodes; fix DAT-080 missing `promotion` ask kind; add `name`/`kind`/`title`/`description` columns to spec tables; add runs and messages table definitions to data model spec; fix duplicate v2.64 entry in CHANGELOG; add schema.sql guidance to DEVELOPMENT.md and CONTRIBUTING.md. Lint green, all tests pass.
