...
*Version v2.64 · 2026-09-04 — change history: [CHANGELOG.md](CHANGELOG.md)*
...
- [3. Architecture](#3-architecture)
  - [Topology: control plane + execution nodes](#topology-control-plane--execution-nodes)
- [4. The Company DNA](#4-the-company-dna)
  - [4.1 Content model](#41-content-model)
  - [4.2 Read path — how coherence actually happens](#42-read-path--how-coherence-actually-happens)
  - [4.3 Write path — learning without corruption](#43-write-path--learning-without-corruption)
  - [4.4 Governance](#44-governance)
  - [4.5 Storage](#45-storage)
  - [4.6 Knowledge vs. operational data (systems of record)](#46-knowledge-vs-operational-data-systems-of-record)
- [5. The org model: humans + agents as members](#5-the-org-model-humans--agents-as-members)
  - [5.1 Initiatives — from directive to coordinated execution](#51-initiatives--from-directive-to-coordinated-execution)
- [6. Spawning & lifecycle](#6-spawning--lifecycle)
  - [6.1 The spawn request (a guarded tool + console action)](#61-the-spawn-request-a-guarded-tool--console-action)
  - [6.2 Policy engine (hard-coded, not prompt-enforced)](#62-policy-engine-hard-coded-not-prompt-enforced)
  - [6.3 Lineage](#63-lineage)
  - [6.4 Personal assistants (deployment shape)](#64-personal-assistants-deployment-shape)
...
- [13. Risks & mitigations](#13-risks--mitigations)
  - [13.1 Residual risk — accepted boundaries, deferred parameters](#131-residual-risk--accepted-boundaries-deferred-parameters)
- [14. Key open decisions](#14-key-open-decisions)
...
- **Stack** (v2.49 re-host: the backend moves Node 22/TypeScript → Java 21 LTS + Spring Boot 3.4;
  console, data plane, and every §8 behavior unchanged): Java 21 LTS + Spring Boot 3.4 daemon
...
- **Quotas & caps**: max concurrent ephemeral workers per spawner, global spawn depth (default 2),
  org-wide concurrent agents (default 100 active agents, persistent + ephemeral combined — a
  runaway backstop sized to trip well before resource exhaustion), per-spawn and org-wide spend
  caps metered by the spend ledger.
...
> in this repo (provenance note up top: the spec suite is the completion of record for these
> sections). Before Phase 0 starts, inline or link the carried-over v1 specs here. If any v1
...
POST /nodes/:id/heartbeat · POST /nodes/:id/claims · POST /nodes/:id/work/pull ·
               ...
               heartbeat owns last_heartbeat + capability state, the claim is the §3 epoch
               fence's mediated boundary — a stale epoch refused here — work pull serves the
               workspaces the node holds live claims on, run report lands results, artifacts,
               and spend lines; a revoked node is refused at every one of these)
...
|---|---|---|---|
| **0. Foundations** | Repo, CI, single-process skeleton | Monorepo, Java 21 + Spring Boot 3.4 skeleton (fat jar), sqlite-jdbc SQLite (WAL), React+Vite console shell (TS strict), REST+WS, 3-OS CI matrix, OCI images under rootless Podman + kind-on-Podman Kubernetes CI (§14.3) | 1 wk |
...
Thirty-five edge-case sweeps (v2.9–v2.43 — every entry in CHANGELOG.md) closed the
enumerated edge-case space inline: each sweep's residue now has a working mechanism in
the section its changelog entry cites, and the spec suite (`specs/`) is the testable
statement of those mechanisms. What remains is stated, not hidden:
