...
- **Always injected**: the org snapshot (who's who), the glossary slice relevant to the task's
  domain, and all *applicable rules* for the workspace's domains.
...
dna_proposals  (id, kind 'card'|'rule'|'decision'|'edit', payload json, proposed_by member,
                 provenance json, status 'open'|'published'|'rejected', reviewed_by?, at)
asks           (id, kind 'approval'|'question'|'assignment'|'spawn_request', from member, to member,
                 payload json, status 'pending'|'answered'|'expired', deadline, created_at)   -- supersedes approvals
board_tasks    + assignee_human_id?   (assignee may now be any member)
spend_ledger   (id, member_id, run_id?, spawn_id?, tokens_in/out, cost, at)
```
...
- **8.1 Agent runtime** — unchanged core (prompt assembly → guarded loop → structured result), with
  ...
  `auto_deny` (default) | `queue_until_morning` (Task Board digest) | `escalate_im`. Scope
...
- **8.9 Console screens** — v1 set plus: **10. Org & People** (members, RBAC, lineage graph,
  ...
  quotas, spend dashboard, spawn audit).
...
- Secrets in OS-encrypted storage (Tauri `safeStorage`); redaction before any egress to providers.
...
| **5. Playbooks** | Multi-stage orchestration | DSL + sandbox, statuses, askUser → Asks, read-only canvas, versions, playbook triggers | 3–4 wks |
| **6. Multi-human org** | A company, not a person | Server deployment, human accounts + RBAC, asks routing + digests, shared board, node registration & workspace-affinity scheduling | 3–4 wks |
| **7. Spawning** | The org flexes | Ephemeral workers (quota, TTL, fold-back memory), then persistent hires (owner approval), policy engine, lineage graph, spend ledger + circuit-breaker | 2–3 wks |
| **8. Production** | Shippable v1 | Tauri shell, encrypted secrets, backup/restore, telemetry (opt-in), docs, installers, security review | 2 wks |

**Total: ~23–30 weeks solo** (compress ~40% with AI-assisted development). Milestones: working
agent week 6 · DNA-coherent company ~week 13 · multi-human org ~week 19 · full spawning ~week 23.
